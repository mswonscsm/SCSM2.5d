...
USE OMP_LIB
      ...
      integer :: ncore, iTimes1, iTimes2, rate  
      ...
      allocatable FTS,FTS1, FTG,FTG1,FST,FGT,FTG2
...
!
!------------------------------------------
      ALLOCATE(FTG(NSR,13,NT),FTG1(NSR,13,NT),FTS(4,NT),FTS1(4,NT),FTG2(NSR,13,NT))
      Allocate(FGT(8,NSR,13,NT),FST(8,4,NT))
      
      FTG=0.0;FTS=0.0
      FGT=0.0;FST=0.0; FTG2=0.0
...
!=2.5D case
!$omp Parallel DEFAULT(SHARED) PRIVATE(ik,wky1,FKY1,FTG1)
     
!$OMP DO
  do ik=myid+1,nky,ntasks
...
MAB,DMAX,CMAX,Efact,VTX,VTN,FTG1,NSR,I_VISCO, IREC,Nani, Nsls,tau_ep, tau_sig,tau_TTI)
    ...
    !$OMP CRITICAL
    FTG2=FTG2+WKY1/PI*FTG1*cos(0.d0*FKY1);
    FTG1(1:NSR,1:13,1:NT)=0.d0
    !$OMP END CRITICAL
...
do ii=1,NSR
	call MPI_REDUCE(FTG2(II,1:13,1:NT),FTG(II,1:13,1:NT),13*NT, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD,ierr)
enddo
...
enddo
    ...
    if(myid==0)write(*,*)'TIME= ', real(iTimes2-iTimes1)/real(rate)/60.d0,'min'
